# Investigation Notes — Lab 60 Deleted Script Recovery Investigation

## Investigation Overview

The investigation examined whether a PowerShell script could be reconstructed after it was deleted from the filesystem.

A harmless script was created, executed, documented, and then removed. The investigation then relied on Sysmon, PowerShell logging, command history, metadata, and previously recorded hashes to reconstruct the activity.
...
The investigation demonstrated that a deleted script can remain forensically visible through historical telemetry.

The strongest surviving evidence was PowerShell Event ID 4104, supported by Sysmon Event ID 11, Sysmon Event ID 1, command history, and the pre-deletion file metadata.

## Final Conclusion

The script was successfully reconstructed as a controlled laboratory artifact even though the physical file was deleted.

The investigation therefore demonstrated:
...
Historical Telemetry
    =
Recoverable Investigation Context
```

This is the central forensic lesson of the lab.
